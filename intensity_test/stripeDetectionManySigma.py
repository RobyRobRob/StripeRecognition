...
def plot_color(arr, name):
    ...
    plt.grid(True)
    y_scan = arr.shape[0]//2
    Img_line = arr[y_scan, :, 0]
    plt.plot(Img_line, "r-")
    Img_line = arr[y_scan, :, 1]
    plt.plot(Img_line, "g-")
    Img_line = arr[y_scan, :, 2]
    plt.plot(Img_line, "b-")
    Img_grayline = cv2.cvtColor(arr, cv2.COLOR_RGB2GRAY)[y_scan]
    plt.plot(Img_grayline, "k-")
...
def read_image(img_name):       # Img einlesen
    ...
    if save_plot:
        plot_array(I_GRAY, "img" + str(test_img_num) + "_Original.png", False)
    return I_GRAY, I_BGR
...
I_max = I_RGB[y_scan].max(axis=1)
peaks, props = find_peaks(I_max, distance=10, height= 30 )
print(f"Gefundene Peaks: {len(peaks)}")
mark_stripes()


if save_plot:
    I_RGB_line = plot_color(I_RGB, "Color_Values")
    width = I_RGB_line.shape[0]
    plt.figure(figsize=(width/10, 5))
    I_RGB_stripe = np.stack([I_RGB_line] * 50, axis=0)
    
    plt.imshow(I_RGB_stripe, cmap=None)
    plt.gca().invert_yaxis()
    plt.plot(I_max, color="k", linewidth=1)
    for x in range(len(peaks)):
        plt.plot(peaks[x], I_max[peaks[x]] , 'rx', markersize=3, markeredgewidth=1)
    folder = os.path.dirname(os.path.abspath(__file__))
    img_path = os.path.join(folder, "images/" + "Plot_On_Img")
    plt.savefig(img_path)